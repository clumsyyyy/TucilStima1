public class Matcher {
    ...
    public static Exec checkHR(Matrix m, String[] keyword, int i, int j){ //horizontal right, ex NEPTUNE
        ...
        while (count < keyword.length && flag && (j + count <= m.cols)){
            compCount++;
            // System.out.println("test " + m.buffer[i][j + count - 1].oriChar + " " + keyword[count]);
            if (!m.buffer[i][j + count].oriChar.equals(keyword[count])){
                flag = false;
            } else {
                count++;
        ...
        //System.out.println(j + (keyword.length - 1));
    ...
    public static Exec checkDRU(Matrix m, String[] keyword, int i, int j){ //diagonal right upper
        ...
        //System.out.println(count + " " + keyword.length + " " + (i - (keyword.length - 1)) + " " + (j + (keyword.length - 1)));
